...
if __name__ == '__main__':
    ...
    # analysis_bpe_tokens_graph_nodes(os.path.join(output_dir, 'all_files_intersection_res/each_instances_res'), output_dir)
    # exit()


    is_top_n_bpc_contain_graph_nodes( os.path.join(output_dir, 'all_files_intersection_res/each_instances_res'), output_dir)
    exit()
    ...
    for file_id in tqdm(root_dir_list, desc="Processing files"): # file_id_list
        ...
        pcpp_bpe_matched, pcpp_bpe_unmatched = count_pcpp_equal_to_linevul_tokenizer(pcpp_tokens_offsets, linevul_token_scores_dict[file_id+".pkl.xlsx"])

        pcpp_bpe_matched_res.append({"file_id":file_id,
                                     "pcpp_bpe_matched":pcpp_bpe_matched,
                                     "pcpp_bpe_unmatched":pcpp_bpe_unmatched,
                                     "matched_rate": pcpp_bpe_matched/ (pcpp_bpe_matched + pcpp_bpe_unmatched)})
        ...
        one_files_intersection_res = calculate_intersection_between_linevul_tokens_socre_and_xfg_nodes(linevul_token_scores_dict[file_id+".pkl.xlsx"],
                                                                          black_ranges)



        all_files_intersection_res = all_files_intersection_res + one_files_intersection_res
        one_files_intersection_res_df = pd.DataFrame(one_files_intersection_res)

        one_files_intersection_res_df.to_excel(os.path.join(output_dir, 'all_files_intersection_res/each_instances_res', file_id+".xlsx"), index=False)

        print("one_files_intersection_res_df:", one_files_intersection_res_df)
    print("all_files_intersection_res_len:", len(all_files_intersection_res))

    analysis_pcpp_bpe_res(pcpp_bpe_matched_res)
    pcpp_bpe_matched_res_df = pd.DataFrame(pcpp_bpe_matched_res)
    pcpp_bpe_matched_res_df.to_excel(os.path.join(output_dir ,"all_files_intersection_res", 'pcpp_bpe_matched_res_df.xlsx'))




    all_files_intersection_res = pd.DataFrame(all_files_intersection_res)
    all_files_intersection_res.to_excel(os.path.join(output_dir ,"all_files_intersection_res", "all_files_intersection_res.xlsx"), index=False)

    # Step 2: 按 bucket_range 分组，汇总 matched_count 和 unmatched_count
    agg_df = all_files_intersection_res.groupby("bucket_range")[["matched_count", "unmatched_count"]].sum().reset_index()
    ...
    # print("agg_df:", agg_df)
    agg_df.to_excel(os.path.join(output_dir ,"all_files_intersection_res", "all_files_intersection_res_agg_analysis_res.xlsx"), index=False)
