...
def is_label_positive_and_pred_positive(data):
    """
    判断一个 SHAP 保存的 pkl 数据是否满足：
        - label == 1
        - predicted_class 表示 positive（兼容多种写法）

    输入：
        data: 从 pkl 读出的 dict，至少包含：
              - data["label"]
    ...
    # 2) predicted_class 必须属于 positive
    pred = data.get("predicted_class", None)

    # 支持的 positive 类型：
    #   - 字符串: "POSITIVE", "positive", "LABEL_1", "label_1"
    #   - 数字: 1
    #   - 布尔: True
    if isinstance(pred, str):
        if pred.upper() in ["POSITIVE", "LABEL_1"]:
            return True
...
def get_label_positive_predict_positive_id():
    ...
    """
    shap_token_scores_dir = "/scratch/c00590656/vulnerability/data-package/models/CodeBERT/scripts/saved_models_msr_processed_func/shap_token_weights"
...
def process_slicing_file(
    ...
    for node_index, node_corr_information in one_slicing_res.items():
        ...
        if "#" in location_updated:
            node_mapped_linevul_tokens = [{"start_offset": int(location_updated.split("#")[0]),
                                           "end_offset": int(location_updated.split("#")[1]),
                                           "token": code_updated,
                                           "weight":0}]
            node_mapped_linevul_tokens_concat = node_mapped_linevul_tokens_concat + node_mapped_linevul_tokens
    ...
    for dirpath, dirnames, filenames in os.walk(file_id_dir):
        for filename in filenames:
            ...
            if not node_mapped_linevul_tokens_concat==[] and node_mapped_linevul_tokens_concat is not None:
                file_id_only = file_id.split(".")[0]
                if file_id_only in linevul_token_scores_dict:
                    slicing_linevul_scores_x_times, slicing_num = process_linevul_groups(
                        node_mapped_linevul_tokens_concat,
                        linevul_token_scores_dict[file_id_only],
                    )
                    full_write_path = full_path.replace("XFG_LineVul_Map", "XFG_Intersection_Visual_X_Times_ReVeal_Map")
                    ...
                    with open(full_write_path, "w") as f:
                        json.dump({"file_id":file_id,
                                        "original_code":original_code,
                                        "ReVeal_Tokens": linevul_token_scores_dict[file_id_only],
                                        "slicing_node_mapped_ReVeal_tokens_concat":node_mapped_linevul_tokens_concat,
                                        "slicing_CodeBERT_scores_x_times":slicing_linevul_scores_x_times,
                                 }, f)
...
def process_linevul_groups(
    ...
    for idx, token_info in enumerate(linevul_token_scores):
        ...
        token_info["shap_group_id"] = shap_group_id
...
def main():
    ...
    file_id_list, reveal_token_scores_dict = read_reveal_result(reveal_result_dir)
    print("Loaded ReVeal SHAP tokens for", len(file_id_list), "files")
    ...
    for file_id in tqdm(root_dir_list, desc="Processing file_ids"):
        process_one_file_id(
            ...
            linevul_token_scores_dict=reveal_token_scores_dict,
        )
...
def save_tex_and_image(json_path: str, tex: str, output_format: str = "png"):
    ...
    tex_path = json_path.replace("XFG_Intersection_LaTeX", "XFG_Intersection_Visual_X_Times_ReVeal_Map") + ".tex"
    tex_dir = os.path.dirname(tex_path)
...
def generate_lipics_latex_from_json(json_path: str, output_format: str = "png"):
    ...
    slicing_tokens = data.get("slicing_node_mapped_ReVeal_tokens_concat", [])
    linevul_tokens = data.get("slicing_CodeBERT_scores_x_times", []) # slicing_linevul_scores_x_times 是linevul_tokens含有 x time组别的版本
...
if __name__ == "__main__":
    ...
    json_root = "/scratch/c00590656/vulnerability/DeepWukong/src/data/msr/XFG_Intersection_Visual_X_Times_ReVeal_Map"
