...
def draw_code_with_underlines_scaled_bak(text,
                                     green_ranges,
                                     black_line_numbers,
                                     full_path,
...
def draw_code_with_underlines_scaled(text,
    ...
    """
    在代码里画：
      - 绿色直线下划线（针对 green_ranges 指定的字符索引）
      - 黑色波浪下划线（针对 wavy_ranges 指定的字符索引）
      - 整行红色下划线（针对 flaw_line_index_dict[file_id]["flaw_line_index"]）
      - 整行黑色下划线（针对 black_line_numbers）
    并将图片与元数据保存到 disk。
    """
    ...
    red_line_numbers = flaw_line_index_dict[int(file_id)]["flaw_line_index"]
    green_indices = expand_ranges(green_ranges)
    wavy_indices  = expand_ranges(wavy_ranges)
    ...
    # 下划线的垂直偏移量设置
    underline_offset = line_spacing * 0.3      # 单条下划线距离字符 baseline 的偏移
    doubleline_gap   = line_spacing * 0.2      # 双行下划线之间的垂直间距
    ...
    for line_no, line in enumerate(lines, start=1):
        ...
        for ch in line:
            ...
            # 3.2 黑色波浪下划线
            if char_ix in wavy_indices:
                # 在字符宽度区间 [x, x + char_width] 上绘制一小段正弦波
                xs = np.linspace(x, x + char_width, 10)
                ys = (y - underline_offset) + 0.002 * np.sin(np.linspace(0, 2 * np.pi, 10))
        ...
        # 3.4 整行黑色下划线
        if line_no in black_line_numbers:
            offset = -underline_offset - doubleline_gap - 0.01  # 再往下错开一点
            ax.hlines(y + offset, 0.0, x,
                      color="black", linewidth=1.5,
                      zorder=5)
    ...
    # 4. 调整坐标范围，避免下划线被裁剪
    bottom_y = y - underline_offset - doubleline_gap - 0.05   # 留点余量
    ax.set_xlim(0, 1)
    ax.set_ylim(bottom_y, 1.05)

    plt.tight_layout()

    # 5. 保存图片与对应的 pkl / json 元数据
    full_path_jpg  = full_path.replace("XFG", "XFG_Intersection_Visual") + ".jpg"
    full_path_pkl  = full_path.replace("XFG", "XFG_Intersection_Visual") + ".pkl"
    full_path_json = full_path.replace("XFG", "XFG_Intersection_Visual") + ".json"
    print("full_path_jpg:", full_path_jpg)
...
def get_all_c_files(root_dir):
    ...
    # 加进度条
    for dirpath, f in tqdm(file_list, desc="Scanning files"):
        file_id = f.split('_')[0]
        ...
        for dirpath, dirnames, filenames in os.walk(file_id_dir):
            for filename in filenames:
                ...
                if str(file_id) in reveal_nodes_weight_importance_info:
                    print("reveal str(file_id):", str(file_id))
                    wavy_ranges = reveal_nodes_weight_importance_info[str(file_id)]
                    draw_code_with_underlines_scaled(original_code, green_ranges, wavy_ranges, xfg_nodes_line_number, full_path, flaw_line_index_dict, file_id)
