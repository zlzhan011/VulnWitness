...
class DataEntry:
    def __init__(self, _id, datset, num_nodes, features, edges, target, file_name, code=False, node_tokens=False, node_code=False):
        self._id = _id
...
def _process_entry(i_entry):
    # 现在每个进程里都有全局的 _global_ds
    dataset, i, entry = _global_ds, *i_entry
    ...
    return DataEntry(
        ...
        target=target,
        file_name=entry["file_name"],
        code=entry.get("code", False),
        node_tokens=entry.get("node_tokens", False),
        node_code=entry.get("node_code", False),
    )
...
class DataSet:
    ...
    def read_data_no_parallel(self, all_src):
    ...
    def read_data_parallel(self, all_src):
        batch_size, num_workers = 500, 16

        # 先统计总行数
        with jsonlines.open(all_src) as r0:
            total = sum(1 for _ in r0)
        ...
        with ctx.Pool(
                ...
                initargs=(self,),  # 只 pickle 一次 self
        ) as pool, tqdm(total=total, desc="Total") as pbar:

            def iter_idx_entry():
                for i, e in enumerate(jsonlines.open(all_src)):
                    yield (i, e)

            for example in pool.imap_unordered(_process_entry, iter_idx_entry(), chunksize=50):
                self.all_examples.append(example)
                pbar.update(1)

    def read_dataset(self, all_src):
        # self.all_examples = self.read_data_no_parallel( all_src)
        self.read_data_parallel( all_src)
