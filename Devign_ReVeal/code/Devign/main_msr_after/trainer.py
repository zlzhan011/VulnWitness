...
def train(model, dataset, max_steps, dev_every, loss_function, optimizer, save_path, log_every=50, max_patience=5, args=None):
    debug('Start Training')
    train_losses = []
    best_model = None
    patience_counter = 0
        ...
        for step_count in tqdm(range(max_steps),desc='training steps'):
            ...
            if step_count % dev_every == (dev_every - 1):
                ...
                if valid_f1 > best_f1:
                    ...
                    _save_file_f1 = open(save_path + '-model_'+str(valid_f1)+'.bin', 'wb')
                    torch.save(model.state_dict(), _save_file_f1)
