...
import dgl
import tqdm
from data_loader.dataset_after_process import DataSet
DataSet.get_next_test_batch_v2
print("dataset_after_process")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir',
                        default='/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/models/Devign_ReVeal/code/data/MSR/predict_result'
    ...
    v0_reproduce = True
    if v0_reproduce:
        args.input_dir = '/data1/cs_lzhan011/vulnerability/data-package/models/Devign_ReVeal/code/data/MSR/full_experiment_real_data_processed'
        input_dir = args.input_dir
        args.output_dir = '/data1/cs_lzhan011/vulnerability/data-package/models/Devign_ReVeal/code/data/MSR/output_dir/predict_result'
        processed_data_path = os.path.join(args.input_dir, 'processed_all_data_binary_v0.bin')
        all_src = os.path.join(args.input_dir, args.dataset + '-full_graph_v0.jsonlines')
    ...
    if  os.path.exists(processed_data_path):
        ...
        print("Reproduce processed.bin again")
    ...
    model_dir = "../hpc/models2/MSR/ggnn/vMSR/1234"
    if args.train:
        ...
        train(model=model, dataset=dataset, max_steps=1000000000, dev_every=64, log_every=None,
              ...
              save_path=model_dir + '/GGNNSumModel', max_patience=100000)
    ...
    #
    #     print("\n\n")
    #     print('Holdout Accuracy: %0.2f\tPrecision: %0.2f\tRecall: %0.2f\tF1: %0.2f' % (acc, pr, rc, f1))
    #     print('=' * 100)
    ...
    #         nextbatch = dataset.get_next_holdout_batch
    #     elif args.holdout_test:
    #         nbatch_train = dataset.initialize_train_batch()
    #         nextbatch_train = dataset.get_next_train_batch
    #     model.eval()
    #     with torch.no_grad():
    #         _loss = []
    #         all_predictions, all_targets = [], []
    #         for _ in range(nbatch_train):
    #             graph, targets = nextbatch_train()
    #             if has_cuda:
    #                 targets = targets.cuda()
    #             predictions = model(graph, cuda=has_cuda)
    #             batch_loss = loss_function(predictions, targets)
    ...
    #
    #     print("\n\n")
    #     print('Training Accuracy: %0.2f\tPrecision: %0.2f\tRecall: %0.2f\tF1: %0.2f' % (acc, pr, rc, f1))
    #     print('=' * 100)
    # print("Training dataset evaluation finishing")
    # exit()
    ...
    if args.holdout or args.holdout_test:
        ...
        if has_cuda:
            ...
            map_location  = torch.device('cpu')
        save_path = '../hpc/models2/MSR/ggnn/vMSR/1234/GGNNSumModel' + '-model.bin'
        ...
        with torch.no_grad():
            ...
            for _ in tqdm(range(nbatch)):
                ...
                if predictions.ndim == 2:
                    ...
                    all_logit.append(predictions.numpy().tolist()[0])
                    all_predictions.extend(
                        predictions.ge(torch.ones(size=predictions.size()).fill_(0.5)).to(
                            dtype=torch.int32).numpy().tolist()
                    )
            ...
            acc, pr, rc, f1 = accuracy_score(all_targets, all_predictions) * 100, \
                              ...
                              recall_score(all_targets, all_predictions) * 100, \
                              f1_score(all_targets, all_predictions) * 100
            print('Run test Holdout Accuracy: %0.2f\tPrecision: %0.2f\tRecall: %0.2f\tF1: %0.2f' % (acc, pr, rc, f1))
            ...
            df = pd.DataFrame({'targets': all_targets, 'predictions': all_predictions, 'files_name': all_files_name, 'logit':all_logit})
            df.to_excel(os.path.join(args.output_dir, 'reveal', 'predict_result.xlsx'))
        ...
        if run_after:
            ...
            for _ in tqdm(range(nbatch_after)):
            ...
            df = pd.DataFrame({'targets': all_targets, 'predictions': all_predictions, 'files_name': all_files_name, 'logit':all_logit})
            df.to_excel(os.path.join(args.output_dir, 'reveal', 'predict_result_after.xlsx'))
        ...
        if run_before:
            ...
            df.to_excel(os.path.join(args.output_dir, 'reveal', 'predict_result_before.xlsx'))
