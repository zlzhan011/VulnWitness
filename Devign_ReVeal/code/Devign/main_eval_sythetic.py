...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='Synthetic_V3', type=str, help='Name of the dataset for experiment.')
    parser.add_argument('--input_dir',
                        default='/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/models/Devign_ReVeal/code/data/Synthetic_V3/full_experiment_real_data_processed',
                        type=str, help='Input Directory of the parser')
    parser.add_argument('--fold', default='Synthetic_V3', help='Which fold of the dataset to load')
    ...
    parser.add_argument('--save_after_ggnn', default=True, action="store_true", help='Evaluate all examples and export them')
    ...
    model_dir = "/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/models/Devign_ReVeal/code/hpc/models2/MSR/ggnn/vMSR/1234"
    model_dir = '/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/models/Devign_ReVeal/code/hpc/models_devign_bak/MSR/devign/vMSR/1234'
    ...
    if args.holdout or args.holdout_test:
        ...
        # save_path = '/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/models/Devign_ReVeal/code/hpc/models2/MSR/ggnn/vMSR/1234/GGNNSumModel' + '-model.bin'
        save_path = '/data/cs_lzhan011/data/cs_lzhan011/vulnerability/data-package/models/Devign_ReVeal/code/hpc/models_devign_bak/MSR/devign/vMSR/1234/GGNNSumModel' + '-model.bin'
    ...
    if args.eval_export:
        ...
        model.eval()
        with torch.no_grad():
            all_predictions, all_probs, all_targets, all_files_name, all_indices = [], [], [], [], []
            index_start = 0
            writer = None
            examples_written = 0
            ...
            for batch_i in tqdm.tqdm(range(num_batches), desc="pass through data"):
                graphs, targets, files_name = dataset.get_next_all_batch_synthetic()
            ...
            assert len(all_targets) == len(all_files_name)

            if writer is not None:
                writer.close()

            all_files_index = [re.split("_", name)[0] for name in all_files_name]
            df = pd.DataFrame(data={
                "prediction": all_predictions,
                "prob": all_probs,
                "target": all_targets,
                "file_name":all_files_name,
                "file_index": all_files_index
            }, index=all_indices)
            ...
            df = df.reset_index().sort_values("file_index")

            df_new = []
            for file_index_original in range(500):
                match_flag = 0
                for index, row in df.iterrows():
                    file_index = row['file_index']
                    if int(file_index) == int(file_index_original):
                        df_new.append(row)
                        match_flag = 1
                if match_flag == 0:
                    df_new.append(pd.Series({
                        "prediction": "",
                        "prob": "",
                        "target": "",
                        "file_name": "",
