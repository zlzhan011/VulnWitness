...
python -u $dataset_dir/Devign/main_eval_msr_multi_category_cross_project_evaluate.py --dataset MSR --input_dir $dataset_dir/data/MSR/full_experiment_real_data_processed --fold $fold --seed $seed \
    --model_type devign --node_tag node_features --graph_tag graph --label_tag targets --fold_dir $fold_dir  --batch_size 256   --save_after_ggnn 2>&1 | tee "$(basename $0)_test_reveal_${fold}_${seed}_${fold_dir}_resample_v2_P50.log"
