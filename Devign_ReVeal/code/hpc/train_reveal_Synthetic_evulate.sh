...
python -u $dataset_dir/Devign/main_eval.py --dataset Synthetic_V3 --input_dir $dataset_dir/data/Synthetic_V3/full_experiment_real_data_processed --fold $fold --seed $seed \
--model_type ggnn --node_tag node_features --graph_tag graph --label_tag targets --batch_size 1  --eval_export --save_after_ggnn 2>&1 | tee "$(basename $0)_ggnn_${fold}_${seed}.log"
retVal=$?
if [ $retVal -ne 0 ]; then
    echo "Error"
    exit $?
