...
def extract_graph_data(project, portion):
    ...
    base_dir='../data'
    base_dir = '/scratch/c00590656/vulnerability/data-package/models/Devign_ReVeal/code/data/'
    ...
    files = os.listdir(os.path.join(data_dir, 'raw_code'))
    file_ids = [file.split("_")[0] for file in files]
    # 计算每个元素的出现次数
    from collections import Counter
    counter = Counter(file_ids)

    # 找出出现次数超过一次的元素
    duplicate_file_ids = [item for item, count in counter.items() if count > 1]
    # duplicate_file_ids = duplicate_file_ids[:1000]
    print("len duplicate_file_ids", len(duplicate_file_ids))
    print(duplicate_file_ids)
    ...
    with jsonlines.open(os.path.join(base_dir, project, "full_experiment_real_data/data.jsonlines")) as reader, jsonlines.open(os.path.join(output_dir, project + '-' + portion + '.jsonlines'), "w") as writer:
        for data in tqdm.tqdm(reader):
            fidx = data['id']
            idid = data['file_name'].split("_")[0]
            # print(idid, "   ",idid)
            # if str(idid) in duplicate_file_ids:
            #     print("idid:", idid)
            # else:
            #     continue
...
def extract_line_graph_data(project):
    ...
    # 找出出现次数超过一次的元素
    duplicate_file_ids = [item for item, count in counter.items() if count > 1]
    duplicate_file_ids = duplicate_file_ids[:100]
    print("len duplicate_file_ids", len(duplicate_file_ids))
    # exit()
    with jsonlines.open(os.path.join(output_dir, project + '-line-ggnn.jsonlines'), "w") as writer:
        for file_name in tqdm(files):
            one_file_id = file_name.split("_")[0]
            if one_file_id in duplicate_file_ids:
                pass
            else:
                continue
